                        ...
                        <tbody>
                            @foreach($orders as $order)
                                <tr>
                                    ...
                                    <td>{{$order->order_total * 24000}}đ</td>
                                    <td>{{$order->status}}</td>
                                    <td>
                                        <div class="btn-group" role="group" aria-label="Actions">
                                            <form method="GET" action="{{ route('order.update', ['id' => $order->order_id]) }}">
                                                @csrf
                                                <button type="submit" class="btn btn-primary">
                                                    Sửa
                                                </button>
                                            </form>
                                            <form method="POST" action="{{ route('order.destroy', ['id' => $order->order_id]) }}">
                                                @csrf
                                                @method('DELETE')
