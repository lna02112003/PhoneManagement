        ...
        <div class="page group">
            <div class="section">
                <div class="col span_2">
                    ...
                    <form method="POST" action="{{ route('attribute.update', ['product_id' => $product_id, 'attribute_id' => $attribute->attribute_id]) }}">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            ...
                            <select name="attribute_id" id="attribute_id" class="form-control">
                                @foreach ($attributes as $attributes)
                                    <option value="{{ $attributes->attribute_id }}">{{ $attributes->attribute_name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="attribute_name">Attribute Value Name:</label>
                            <input type="text" name="attribute_value_name" id="attribute_value_name" class="form-control" value="{{ $attribute->attribute_value_name }}">
                        </div>
                        <div class="form-group">
                            <label for="price">Price:</label>
                            <input type="text" name="price" id="price" class="form-control" value="{{ $attribute->price }}">
                        </div>
